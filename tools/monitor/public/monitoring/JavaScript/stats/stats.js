...
function getBlockData(number, el)
{
	...
	{
		var transSpans = '<br /><span style="display:inline-block" class="blocksTransactionsHdr" >Transactions:</span>'
...
function scrollBlocksLeft(){
	...
	{
		var toScroll = $('#blockHolder').scrollLeft()
...
function scrollBlocksRight(){
	...
	if($('#blockHolder').scrollLeft() <= $('#blockScroll').width()-$('#blockHolder').width()-27)
	{
		...
		var toScroll = ($('#blockScroll').width()-$('#blockHolder').width())-$('#blockHolder').scrollLeft();
...
function growBlock(el) {
	...
	var height = $(el).parent().find('.height').val()
